<template>
	<div class="byeraccountcheck">
		...
		<div class="bottom" v-for='item in items'>
			...
			<span class="adopt">{{ item.frozenState == 1 ? '已冻结' : item.auditState == 0 ? '待审核' : item.auditState == 1 ? '已通过' :  item.auditState ==2 ? '未通过' : '' }}</span>
		</div>
...
<script>
	...
	export default{
		...
		methods:{
			fanhui(){
				var that = this;
				that.$router.push({name: 'buyinfo',params: {fenye: that.fenye}})
			},
			pictures1(){
				this.pictures = true
			},

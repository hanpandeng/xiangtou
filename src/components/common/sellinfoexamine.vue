<template>
	<div class="sellinfoexamine">
		<div class="title"><img src="../../assets/daohang.png"> 当前位置：账号管理 > <router-link tag='span' to='/advertisement'>接广告账号</router-link>  > 审核</div>
...
<!-- 				<span class="value" v-for='(it,id) in JSON.parse(item.advertiseImgs)'>
					<img @click='pictures1()' :src="it">
					</span>
 -->			</div>
	...
	export default{
		...
		methods:{
			adopt(){
				var that= this;
				axios({
					url:'Sellinfo/setsellerconfirmed',
					method:'post',
					data:{
						sellerInfoId:that.sellerInfoId,  
					sellerAccountId:that.sellerAccountId 
					...
					if(res.data.code == 200){
						that.$router.push({path: '/advertisement',params: {fenye: that.fenye}})
					}
				})	
			},
			Notthrough(){

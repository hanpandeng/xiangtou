<template>
	<div class="addadsense">
		<div class="title"><img src="../../assets/daohang.png"> 当前位置：<router-link tag='span' to='/advertisement'>接广告账号</router-link> > 添加广告位</div>
		...
		<div class="content" v-for='flag,state in itemlist' :index='state'>
			...
			<div class="every" v-for='(item,inde) in items' :index='inde'> 
				<span class="name">{{inde + 1}}屏</span>
				单价:
				<span class="value">{{item.unitPrice}}天/元 </span> 
				销量:
				<span class="value">{{item.pedestrianVolume}} </span> 
				标签:
				<span class="value">
...
<script>
	...
	export default{
		...
		created(){
			var that = this;
			// console.log(that.$route.query.sellerAdvertiseId)
			// console.log(that.$route.query.currentPage)
				axios({
